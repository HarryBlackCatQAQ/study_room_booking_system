# keep the go build stage on a modern golang image that can compile the current module version
FROM golang:1.26-bookworm AS builder

# keep the go build workspace explicit inside the image
WORKDIR /build/services/go-availability-service

# copy the go module metadata first so dependency downloads can use docker layer caching
...
# keep a minimal ca-certificates bundle available for standard library networking
