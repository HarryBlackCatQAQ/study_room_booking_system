# keep the java build stage on maven with temurin 21 to match the current project pom settings
...
# copy the java service maven wrapper and pom files first so dependency resolution can use docker layer caching
...
# build the runnable spring boot jar without running tests during image creation
